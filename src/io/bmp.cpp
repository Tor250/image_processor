...
auto BMP::WriteRow(const Image &image, BinaryWriter &output, size_t row) {
    ...
    for (size_t col = 0; col != image.GetWidth(); ++col) {
        ...
        output << static_cast<uint8_t>(std::clamp(pixel.red, 0.0f, upper));    // NOLINT
        output << static_cast<uint8_t>(std::clamp(pixel.green, 0.0f, upper));    // NOLINT
        output << static_cast<uint8_t>(std::clamp(pixel.blue, 0.0f, upper));    // NOLINT
    }
